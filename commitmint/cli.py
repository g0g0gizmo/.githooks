...
LANG_PATTERNS = {
    ...
    'toml': ('.toml',),
    'markdown': ('.md',),
    'labview': ('.vi', '.vim', '.vit', '.ctl', '.ctt', '.lvclass', '.lvlib', '.lvproj', '.lvlibp'),
}
def run_perl(files):
    if not files:
        return 0
...
def run_js_ts(js_files, ts_files):
    ...
    if which('npx'):
        run(['npx', '--no-install', 'prettier', '--write', *files])
...
@click.command()
@click.option('--staged-only', is_flag=True, default=True, help='Operate only on staged files')
def main(staged_only):
    ...
    status |= run_js_ts(groups['javascript'], groups['typescript'])
