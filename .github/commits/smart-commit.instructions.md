...
> **IMPORTANT:**
>
> Only use Jira Smart Commits if the current branch name matches this pattern:
>
> `<UPPERCASE_INITIALS>_<JIRA-ISSUE-KEY>_short-description`
>
> Example: `JT_PTEAE-1234_fix-bug`
>
> If the branch name does **not** match this pattern, **do not** include Smart Commit commands in your commit messages.
>
> To determine the current branch name, always use git commands (e.g., `git rev-parse --abbrev-ref HEAD`), not editor extensions or UI tools.
---
...
> **Note:** This guide covers Jira-specific Smart Commit commands. For conventional commit format (type, scope, emoji, etc.), see [conventional-commit.instructions.md](conventional-commit.instructions.md).
...
Combine conventional commit format (see [conventional-commit.instructions.md](conventional-commit.instructions.md)) with Smart Commit commands:
...
For commit types, scopes, and emoji usage, refer to [conventional-commit.instructions.md](conventional-commit.instructions.md).
