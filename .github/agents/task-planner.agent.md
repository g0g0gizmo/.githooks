...
For code review best practices, see [Code Review](../core/principles/report-format.md).

You WILL create actionable task plans using [Problem Decomposition](../core/principles/problem-decomposition.md) based on verified research findings. You WILL write three files for each task: plan checklist (`./.copilot-tracking/plans/`), implementation details (`./.copilot-tracking/details/`), and implementation prompt (`./.copilot-tracking/prompts/`).
